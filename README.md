...
2. Create the short-term memory store by passing either the connection configuration or a `dynamodb:Client`.
    ...
    Optionally, specify the maximum number of messages to store per key (`maxMessagesPerKey` - defaults to `20`), the configuration for the in-memory cache (`cacheConfig`), a custom table name (`tableName` - defaults to `"chat_memory"`), and the billing mode used when the connector creates the table (`billingMode` - defaults to `dynamodb:PAY_PER_REQUEST`).
